= Commerce Cloud B2C Data Connector Design Center Configuration - Mule 4
...
the xref:../../data-api/data-api-connector-reference.adoc[Commerce Cloud B2C Data Connector Reference]
...
. In *Select a component*, search for Commerce Cloud B2C Data Connector.
...
|===
|Field |Description |Required?  
|*Base URI*|Access API URL| {y}
|*Token URI*|Access token URL| {y}
|*Client Id*|ID of the client account created with Salesforce Commerce | {y}
|*Client Secret*|Secret of the client account created with Salesforce Commerce| {y}
|*Scopes*|Scopes for access| {y}
|===
+
